...
metadata:
  ...
  source-license: "MIT, Copyright (c) 2025 virgiliojr94"
  version: "1.4.0+01f8a742ae"
---
...
Choose the destination skill root (`SKILLS_HOME`). For **personal** (user-level) installs, default to the Claude Code root `~/.claude/skills` — this copy of the converter runs only in Claude Code, so it writes to the host-private root directly and takes no symlink:
...
Selection rules:
1. Personal install: set `SKILLS_HOME` to `~/.claude/skills` (create the directory if missing). Use `~/.agents/skills` only when the user asks for the cross-agent root; then rule 2 applies.
